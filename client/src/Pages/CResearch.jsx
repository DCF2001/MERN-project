  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formDataWithImages = {
        ...formData,
        imgUrls: imagePreviews, // Add image URLs to formData
      };

      // Upload images
      const imageUploadResponse = await fetch("/api/research/upload", {
        method: "POST",
      ...
      const uploadedImageUrls = await imageUploadResponse.json();
      formDataWithImages.imgUrls = uploadedImageUrls.map((image) => image.url);

      // Create research with image URLs
      const createResearchResponse = await fetch("/api/research/create", {
        method: "POST",
        headers: {
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <span className="text-lg text-green-700">
        Welcome to the Research Hub: Where Ideas Flourish, Innovations Blossom, and Transformative Solutions Emerge for the
        Future of Garbage Management.
      </span>
      ...
      <form className="flex flex-col" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <input type="text" placeholder="Date" className="border p-3 rounded-lg" id="date" required value={formData.date} />

          <select id="category" className="border p-3 rounded-lg" onChange={handleChange} value={formData.category}>
            <option value="" className="">
              Select the Garbage Management Research Categories{" "}
            </option>
        ...
        <br />
        <span className="text-green-700 font-semibold">Select the checkboxes for waste reduction practices you have implemented for this research:</span>
        <br />
        ...
        <div className="flex flex-col flex-1 ">
          <p className="font-bold">
            Images: <span className="font-normal text-gray-900 ml-2">The first page will be the cover (max 6)</span>
          </p>
          <br />
            ...
            <div className="flex gap-4">
              {imageFiles.map((file, index) => (
                <div key={index} className="relative">
                  ...
                  <button type="button" className="absolute top-0 right-0 p-2 bg-red-600 text-white rounded-full" onClick={() => handleDeleteImage(index)}>
                    X
                  </button>
                </div>
              ))}
